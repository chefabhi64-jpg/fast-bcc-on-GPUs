...
void LexSortIndices(
    ...
    CUDA_CHECK(cudaMallocManaged(&d_temp_storage, temp_storage_bytes),
               "Failed to allocate managed temp storage for radix sort");

    // Sort indices based on keys
    cub::DeviceRadixSort::SortPairs(d_temp_storage, temp_storage_bytes, d_merged, d_merged_keys_sorted, d_indices, d_indices_sorted, num_items);

    // Clean up temporary storage
    CUDA_CHECK(cudaFree(d_temp_storage), "Failed to free temp storage");
}
...
void compute_level(
    ...
    CUDA_CHECK(cudaMallocManaged(&d_temp_storage, temp_storage_bytes), "Failed to allocate d_temp_storage");
...
float cuda_euler_tour(
    ...
    CUDA_CHECK(cudaMallocManaged((void **)&notAllDone, sizeof(int)), "Failed to allocate notAllDone");
    ...
    CUDA_CHECK(cudaFree(notAllDone), "Failed to free notAllDone");
