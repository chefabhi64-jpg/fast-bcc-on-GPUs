...
// marked
__global__
void init_parent_label_st_edges(int* temp_label, int* label, uint64_t* st_edges, int V) {
...
// marked
void run_union_find(int* temp_label, int* label, uint64_t* st_edges, uint64_t* edges, int numVert, long numEdges) {
    long grid_size_union = CEIL(numEdges, tb_size);
    union_find_gpu<<<grid_size_union, tb_size>>>(numEdges, temp_label, edges, st_edges);
...
float construct_st(graph_data& d_input) {
    ...
    // std::cout << "AST Construction time: " << dur << " ms." << std::endl;
