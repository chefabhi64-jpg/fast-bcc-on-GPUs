...
void run_union_find_SF(struct graph_data* d_input , int V , long SIZ , int f){
    long grid_size_union = (SIZ + tb_size - 1) / tb_size;

    long total_elt = SIZ;
    union_find_gpu_COO_SF<<<grid_size_union, tb_size>>>(total_elt , *d_input);
}
...
float ComputeTagsAndSpanningForest( struct graph_data* h_input , struct graph_data* d_input , struct graph_data_host* h_input_host){
    ...
    CUDA_CHECK(cudaMemcpy(d_input->size, h_input->size, sizeof(long), cudaMemcpyHostToDevice), "Failed to copy d->input size");
    CUDA_CHECK(cudaMemcpy(d_input->edges, h_input->edges + i, min(batch_size, edges_for_gpu - i) * sizeof(uint64_t), cudaMemcpyHostToDevice), "Failed to copy edges");
    
    run_union_find_SF(d_input , h_input->V[0] , total_elt , bat%2);
    ...
    for(; i < edges_for_gpu ; i += batch_size){
        ...
        CUDA_CHECK(cudaMemcpy(d_input->size, h_input->size, sizeof(long), cudaMemcpyHostToDevice), "Failed to copy size");
        CUDA_CHECK(cudaMemcpy(d_input->edges , h_input->edges + i, total_elt * sizeof(uint64_t), cudaMemcpyHostToDevice), "Failed to copy edges");
