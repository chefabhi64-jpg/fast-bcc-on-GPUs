...
double solveQ_min(int m, int* d_na, int q, int* d_left, int* d_right, int* d_ansmin , int* d_a , int n) {
    ...
    auto dur = Timer.stop();
    CUDA_CHECK(cudaFree(d_lookupmin), "Failed to free");

    return dur;
}

double main_min(int n, int q, int* d_a , int* d_left , int* d_right , int* d_ansmin , int n_asize , int* d_na){
    ...
    dur += solveQ_min(n_asize, d_na, q, d_left, d_right, d_ansmin , d_a , n );
