...
public:
    ...
    int      *first;      // For first occurrence
    int      *last;       // For last occurrence
    int      *low;        // For low  value of each vertex
    int      *high;       // For high value of each vertex
    uint64_t *sf_edges;   // Spanning Forest on cross-edges 
    ...
    graph_data() : parent(nullptr), first(nullptr), last(nullptr),
                   low(nullptr), high(nullptr), sf_edges(nullptr),
                   label(nullptr), comp_head(nullptr), edgelist(nullptr) {}
    ...
    ~graph_data() {
        ...
        if (low)       CUDA_CHECK(cudaFree(low),    "Failed to free low");
        if (high)      CUDA_CHECK(cudaFree(high),   "Failed to free high");
        if (sf_edges)  CUDA_CHECK(cudaFree(sf_edges), "Failed to free flag");
...
inline void print_device_edges(uint64_t* d_edges, long count, const char* label = "Device Edges") {
    ...
    CUDA_CHECK(cudaMemcpy(edges_host, d_edges, count * sizeof(uint64_t), cudaMemcpyDeviceToHost),
               "Failed to copy device edges to host");
